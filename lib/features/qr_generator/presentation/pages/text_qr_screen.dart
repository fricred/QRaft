...
class TextQRScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TextQRScreen> createState() => _TextQRScreenState();
}

class _TextQRScreenState extends ConsumerState<TextQRScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (qrToEdit != null) {
        ...
        final textData = TextData(text: qrToEdit.data);
        ref.read(textFormProvider.notifier).loadFromEntity(textData, qrToEdit.name);
        ref.read(qrCustomizationControllerProvider.notifier).loadFromEntity(qrToEdit);
      } else {
        // CREATE MODE: Reset providers
        ref.read(textFormProvider.notifier).reset();
      ...
      appBar: AppBar(
        ...
        title: Text(
          widget.editingQRCode != null
            ...
            : (AppLocalizations.of(context)?.qrTypeText ?? 'Text Message QR'),
        ...
        children: [
          Expanded(
            child: TabBarView(
              ...
              children: [
                // Form Tab
                TextForm(onContinue: () => _tabController.animateTo(1)),
            ...
            child: Column(
              ...
              children: [
                ...
                PrimaryGlassButton(
                  ...
                  icon: widget.editingQRCode != null
                    ? Icons.check_rounded
                    : Icons.save_rounded,
                  isLoading: _isSaving,
                  onPressed: textState.isValid ? _saveQRCode : null,
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // Expand button
              if (ref.watch(textFormProvider).isValid)
                GestureDetector(
        ...
        if (!colorAccess.isAllowed)
          Positioned.fill(
            child: FeatureLockedOverlay(
              featureName: AppLocalizations.of(context)?.colorCustomization ?? 'Color Customization',
              description: AppLocalizations.of(context)?.colorCustomizationLockedDesc ?? 'Upgrade to Pro to customize QR code colors',
            ),
          ),
      ],
    );
    ...
    return Stack(
      children: [
        ...
        if (!logoAccess.isAllowed)
          Positioned.fill(
            child: FeatureLockedOverlay(
              featureName: AppLocalizations.of(context)?.logoInsertion ?? 'Logo Insertion',
              description: AppLocalizations.of(context)?.logoInsertionLockedDesc ?? 'Upgrade to Pro to add your logo to QR codes',
            ),
          ),
      ],
    );
  ...
  void _saveQRCode() async {
    ...
    // Check QR limit for new QR codes (not edits)
            ...
            SnackBar(
              content: Row(
                children: [
                  ...
                  Text(l10n?.qrUpdatedSuccess(textState.name) ?? 'QR code "${textState.name}" updated successfully!'),
                ],
              ),
              backgroundColor: const Color(0xFF00FF88),
            ...
            SnackBar(
              content: Row(
                children: [
                  const Icon(Icons.check_circle, color: Colors.white),
                  const SizedBox(width: 8),
                  Text(l10n?.qrSavedSuccess(textState.name) ?? 'QR code "${textState.name}" saved successfully!'),
                ],
              ),
              backgroundColor: const Color(0xFF00FF88),
              duration: const Duration(seconds: 3),
